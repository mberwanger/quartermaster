...
description: An unfinished proposal, kept in the store so the thinking is not lost. It is distributed and readable, but no ruleset may turn it into a rule.
domain: engineering
type: concept
status: draft
provenance: asserted
...
active` and a provenance of `verified` or `decided`. Naming this id in a ruleset
fails the build rather than quietly loading an unfinished idea into every
session:

```
ruleset "engineering" references "eng.experimental-cache", which does not meet
